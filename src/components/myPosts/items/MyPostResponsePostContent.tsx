...
import ResponseInfo from "./ReponseInfo";
const MyPostResponsePostContent = ({ caption, user_id, postId }: { caption: string, user_id: string, postId: string }) => {
  ...
  return (
    <View className="bg-amber-50 rounded-2xl p-5 shadow-sm">
      ...
      <TouchableOpacity onPress={() => setShowResponseInfo(true)}>
      ...
      <ResponseInfo
        showResponseInfo={showResponseInfo}
        setShowResponseInfo={setShowResponseInfo}
        postId={postId}
      />
    </View>
  );
